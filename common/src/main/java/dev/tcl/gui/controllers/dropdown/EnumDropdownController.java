...
public class EnumDropdownController<E extends Enum<E>> extends AbstractDropdownController<E> {
    ...
    public EnumDropdownController(Option<E> option, ValueFormatter<E> formatter, Collection<? extends E> values) {
        super(option, (values == null ? Arrays.stream(option.pendingValue().getDeclaringClass().getEnumConstants()) : values.stream()).map(formatter::format).map(Component::getString).toList());
