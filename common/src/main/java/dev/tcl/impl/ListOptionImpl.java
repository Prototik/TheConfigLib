...
public final class ListOptionImpl<T> implements ListOption<T> {
    ...
    @Override
    public void insertEntry(int index, ListOptionEntry<T> entry) {
    ...
    void callListeners(boolean bypass) {
        ...
        if (bypass || listenerTriggerDepth == 0) {
            if (listenerTriggerDepth > 10) {
                throw new IllegalStateException("Listener trigger depth exceeded 10! This means a listener triggered a listener etc etc 10 times deep. This is likely a bug in the mod using " + TheConfigLib.SHORT_NAME + "!");
            }
            ...
            for (BiConsumer<Option<List<T>>, List<T>> listener : listeners) {
                try {
                    ...
                    TheConfigLib.LOGGER.error("Exception whilst triggering listener for option '%s'".formatted(name.getString()), e);
                }
            }
